...
class RecipeModifier:
    ...
    def apply_modification(
        ...
        modification: ModificationObject,
        validate: bool = True
    ) -> Tuple[Optional[Recipe], List[ChangeRecord]]:
        """
        Apply a complete modification to a recipe with optional safety validation.

        Args:
            recipe: Original recipe to modify
            modification: Structured modification to apply
            validate: Whether to validate modification safety first (default: True)

        Returns:
            Tuple of (modified_recipe, all_change_records) or (None, []) if validation fails
        """
        # Optional: Validate modification safety first
        if validate:
            is_safe, warnings = self.validate_modification_safety(modification, recipe)
            ...
            if not is_safe:
                logger.error(f"Modification validation failed. Skipping modification: {modification.modification_type}")
                return None, []
            
            logger.info(f"Modification passed safety validation: {modification.modification_type}")
    ...
    def apply_modifications_batch(
        ...
        """
        Apply multiple modifications to a recipe sequentially with optional validation.

        Args:
            recipe: Original recipe to modify
            modifications: List of modifications to apply
            validate: Whether to validate each modification safety first (default: True)
        ...
        logger.info(f"Applying {len(modifications)} modifications sequentially (validation: {validate})")

        for i, modification in enumerate(modifications):
            logger.info(f"Applying modification {i + 1}/{len(modifications)}: {modification.modification_type}")

            modified_recipe, change_records = self.apply_modification(
                current_recipe, modification, validate=validate
            )
            
            # If validation is enabled and modification failed, skip it
            if modified_recipe is None:
                logger.warning(f"Skipped modification {i + 1} due to validation failure")
                all_change_records.append([])  # Empty change record for skipped modifications
                continue
            
            current_recipe = modified_recipe
            all_change_records.append(change_records)

        logger.info(f"Applied all valid modifications. Final recipe has {len(current_recipe.ingredients)} ingredients and {len(current_recipe.instructions)} instructions")
